...
class SummaryStatisticsInspectionStrategy_ai(DataInspectionStrategy):
    def inspect(self, df: pd.DataFrame):
        """
        ...
        Returns:
        str: Summary statistics as a formatted string.
        """
        numerical_summary = df.describe().to_string()
        categorical_summary = df.describe(include=["O"]).to_string()

        summary_text = f"Summary Statistics (Numerical Features):\n{numerical_summary}\n\n"
        summary_text += f"Summary Statistics (Categorical Features):\n{categorical_summary}"
